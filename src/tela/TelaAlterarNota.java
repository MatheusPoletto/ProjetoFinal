...
public class TelaAlterarNota extends JInternalFrame implements ActionListener{
	...
	public void preencherCampos(Venda venda, NotaFiscal nf, Double valorTotal){
		...
		dtbProdutos.addRow(new String[]{"1", "Venda de imóvel - ID IMÓVEL: " + String.valueOf(venda.getImovel().getIdImovel()), String.valueOf(venda.getImovel().getValorTotal())});
		}else if(venda.getImovel().getMesesContrato() > 1){
			dtbProdutos.addRow(new String[]{"1", "Aluguel de imóvel - ID IMÓVEL: " + String.valueOf(venda.getImovel().getIdImovel()), String.valueOf(venda.getImovel().getValorMensal() * venda.getImovel().getMesesContrato())});	
		}
		dtbProdutos.addRow(new String[]{"2", "Comissão corretor - ID CORRETOR:" + venda.getCorretor().getIdCorretor(), String.valueOf(venda.getComissaoCorretor())});
		dtbProdutos.addRow(new String[]{"3", "Comissão imobiliária - ID IMOBILIÁRIA: " + String.valueOf(nf.getImobiliaria().getIdImobiliaria()), String.valueOf(venda.getComissaoImobiliaria())});
	}
