...
public class TelaCadastrarVenda extends JInternalFrame implements ActionListener {
	...
	public TelaCadastrarVenda() {
		...
		jlbComissaoImobiliaria = cp.criarLabel("COMISSÃO DA IMOBILIÁRIA COM A VENDA: ", 240, 20, 250, 24,
				jlbComissaoImobiliaria);
		...
		jlbComissaoCorretor = cp.criarLabel("COMISSÃO DO CORRETOR RESPONSÁVEL PELA A VENDA: ", 10, 80, 340, 24,
				jlbComissaoImobiliaria);
	...
	public void preencherCampos(Imovel imovel) {
		...
		jtaImovel.setText("Bairro: " + imovel.getEndereco().getBairro() + " - Cidade: "
				+ imovel.getEndereco().getCidade() + " - CEP: " + imovel.getEndereco().getCep());
		...
		jtaProprietario
				.setText("Nome: " + proprietario.getPessoa().getNome() + " - RG: " + proprietario.getPessoa().getRg());
		jtaProprietario.setText("Tel. Residencial: " + proprietario.getPessoa().getTelefoneResidencial()
				+ " - Tel. Celular: " + proprietario.getPessoa().getTelefoneCelular() + " - Email: "
				+ proprietario.getPessoa().getEmail());
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jbtProcurarCorretor) {
			...
			telaPrincipal.getTlPrincipal().getTlProcurarCorretor()
					.setLocation(telaPrincipal.getTlPrincipal().getTlProcurarCorretor().getX(), 10);
		}
		...
		if (e.getSource() == jbtRegistrarNota) {
			...
			Venda venda = cn.salvarVenda(horaAgora, Double.valueOf(jtfComissaoImobiliaria.getText()),
					Double.valueOf(jtfComissaoCorretor.getText()), imovelVenda, clienteProprietario,
					CorretorResponsavel);
			...
			Double total = Double
					.valueOf(String.format(Locale.US, "%.2f",
							Math.floor(Double.valueOf(jtfValorImovel.getText())
									+ Double.valueOf(jtfComissaoCorretor.getText())
	...
	public void selecionouCorretor(Corretor corretor) {
		...
		jtfNomeCorretor.setText(
				CorretorResponsavel.getPessoa().getNome() + " - RG:" + CorretorResponsavel.getPessoa().getRg());
	}
